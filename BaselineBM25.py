...
def IDF_Generator(corpusfile, delimiter=' ', base=math.e) :
    ...
    for line_raw in open(corpusfile,"r",encoding="utf-8") :
        table = str.maketrans({key: None for key in string.punctuation})
        line = line_raw.translate(table)
        doci = word_tokenize(line)
        doc = []
...
def GetBM25Score(Query, Passage, k1=1.5, b=0.75, delimiter=' ') :
    ...
    global docIDFDict,avgDocLength
    table = str.maketrans({key: None for key in string.punctuation})
    new_query = Query.translate(table)
    new_passage = Passage.translate(table)
    query_words_temp= word_tokenize(new_query)
    passage_words_temp = word_tokenize(new_passage)
    query_words = []
    passage_words = []
    stop_words = set(stopwords.words('english'))
    for w in query_words_temp:
        if w not in stop_words:
            query_words.append(w)
    for w in passage_words_temp:
        if w not in stop_words:
            passage_words.append(w)

    for i in range(len(passage_words)):
        passage_words[i] = passage_words[i].lower()
        passage_words[i] = stemmer.stem(passage_words[i])

    for i in range(len(list(query_words))):
        query_words[i] = query_words[i].lower()
        query_words[i] = stemmer.stem(query_words[i])
...
if __name__ == '__main__' :
    ...
    testFileName = "eval2_unlabelled.tsv"  # This file should be in the following format : queryid \t query \t passage \t passageid # order of the query
